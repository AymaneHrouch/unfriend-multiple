...
(function () {
    ...
    const getFriends = () => document.querySelectorAll('._55wp._7om2._5pxa._8yo0[data-sigil="undoable-action"]');
    ...
    const toUnfriend = () => {
        return Array.prototype.slice.call(document.getElementsByClassName("mycheckbox")).filter(f => f.checked);
    };
    ...
    const unfriend = (id, arr) => {
        ...
        if (id < arr.length) {
            setTimeout(function () {
                friends[friendIndex].children[2].children[0].children[0].children[3].children[0].click();

                document.querySelectorAll('[data-sigil="touchable touchable mflyout-remove-on-click m-unfriend-request"]')[0].click();

                console.log(friends[friendIndex].children[1].children[0].children[0].children[0].children[0].innerText + " has been unfriended");
